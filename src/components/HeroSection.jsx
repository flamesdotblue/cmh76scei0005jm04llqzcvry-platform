  ...
  return (
    <section className="relative h-[58vh] md:h-[68vh] w-full overflow-hidden">
      ...
      <div className="relative z-10 h-full max-w-7xl mx-auto px-4 sm:px-6 flex flex-col items-center justify-center text-center gap-6">
        <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 bg-white/10 backdrop-blur border border-white/10 shadow">
          <span className="text-xs uppercase tracking-wider text-white/80">Dish-centric food search</span>
        </div>
        <h1 className="text-3xl sm:text-5xl font-black leading-tight">
          <span className="bg-gradient-to-r from-orange-400 via-amber-300 to-green-300 bg-clip-text text-transparent">Find, compare, and crave</span> your favorite dishes
        </h1>

        {/* Search and location */}
        <div className="w-full max-w-3xl mx-auto grid grid-cols-1 md:grid-cols-[200px_1fr] gap-3">
          <div className="relative">
            <button className="w-full h-12 rounded-2xl bg-white/10 border border-white/15 hover:border-white/25 transition shadow-inner flex items-center justify-between px-4">
              <span className="flex items-center gap-2 text-white">
                <MapPin className="text-orange-300" size={18} /> Bagalkot
              </span>
        ...
        <div className="mt-2 flex items-center gap-2 flex-wrap justify-center">
          {districts.map((d) => (
            ...
            >
              {d.name}
              {!d.active && (
                <span className="ml-1 rounded-full bg-white/10 text-[10px] px-2 py-0.5">Coming Soon</span>
              )}
            </span>
          ))}
        </div>
